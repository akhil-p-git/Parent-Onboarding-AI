...
# Build secrets list dynamically to exclude empty ARNs
locals {
  api_secrets = concat(
    var.db_secret_arn != "" && var.db_secret_arn != null ? [
    ...
    var.redis_secret_arn != "" && var.redis_secret_arn != null ? [
      { name = "REDIS_HOST", valueFrom = "${var.redis_secret_arn}:host::" },
    ] : []
  )
}
  ...
  container_definitions = jsonencode([
    {
      ...
      environment = concat([
        ...
        { name = "DEBUG", value = var.environment != "production" ? "true" : "false" },
        { name = "PORT", value = tostring(var.container_port) },
      ], var.redis_url != "" && (var.redis_secret_arn == "" || var.redis_secret_arn == null) ? [
        { name = "REDIS_URL", value = var.redis_url },
      ] : [])

      secrets = length(local.api_secrets) > 0 ? local.api_secrets : null
  ...
  container_definitions = jsonencode([
    {
      ...
      environment = concat([
        { name = "APP_ENV", value = var.environment },
        { name = "DEBUG", value = var.environment != "production" ? "true" : "false" },
        { name = "WORKER_TYPE", value = "delivery" },
      ], var.redis_url != "" && (var.redis_secret_arn == "" || var.redis_secret_arn == null) ? [
        { name = "REDIS_URL", value = var.redis_url },
      ] : [])
